-- =====================================================
-- INICIALIZACIÓN COMPLETA DE SUPABASE PARA TECNOBOT SAAS
-- =====================================================
-- 
-- INSTRUCCIONES:
...
-- 4. Ejecuta el script
-- 
-- Este script creará todas las tablas, funciones, triggers y políticas
-- necesarias para el sistema multi-tenant de TecnoBot.
-- =====================================================
...
DROP POLICY IF EXISTS "Members can manage tenant chatbots" ON chatbots;
...
DROP POLICY IF EXISTS "Members can manage tenant flows" ON flows;
...
DROP POLICY IF EXISTS "System can manage analytics" ON analytics_events;
...
DROP POLICY IF EXISTS "Admins can manage tenant webhooks" ON webhooks;
...
DROP POLICY IF EXISTS "System can manage webhook logs" ON webhook_logs;
...
-- Insertar registros en migrations
INSERT INTO migrations (filename, checksum, success) 
VALUES 
    ('init-supabase.sql', 'complete_initialization_v2', true)
...
BEGIN
    ...
    RAISE NOTICE '📊 Tablas creadas: tenants, tenant_users, chatbots, conversations, messages, flows, analytics_events, webhooks, webhook_logs';
